...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    fetchProductions();
    fetchInventoryItems();
  }, [fetchUsers, fetchProductions, fetchInventoryItems]);
  ...
  const {
    ...
    const first = workers.filter((w) => w.position === 'FIRST');
    const second = workers.filter((w) => w.position === 'SECOND');
    const third = workers.filter((w) => w.position === 'THIRD');

    return {
      adminCount: admins.length,
      managerCount: managers.length,
      workerCount: workers.length,
      firstWorkers: first.length,
      secondWorkers: second.length,
      thirdWorkers: third.length,
    };
  }, [users]);
  ...
  const neutralCardClass = cardClass;
  ...
  return (
    <div className="space-y-6">
      ...
      {(usersError || productionsError || inventoryError) && (
        <div className="p-3 text-sm text-red-800 border border-red-200 rounded-md bg-red-50">
          {usersError && <p>User data error: {usersError}</p>}
          {productionsError && <p>Production data error: {productionsError}</p>}
          {inventoryError && <p>Inventory data error: {inventoryError}</p>}
        </div>
      )}

      {(usersLoading || productionsLoading || inventoryLoading) && (
        <p className="text-sm text-gray-500">Loading dashboard data...</p>
      )}

      {/* User role summary */}
      <div className="p-4 border rounded-2xl bg-gradient-to-r from-slate-50 via-white to-blue-50 border-slate-100">
        <h2 className="mb-3 text-sm font-semibold text-gray-700">Users by role</h2>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div className={neutralCardClass}>
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium tracking-wide text-gray-500 uppercase">Admins</p>
            ...
            <p className="mt-1 text-[11px] text-gray-500">Total admin users in the system</p>
          </div>
          <div className={cardClass}>
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium tracking-wide text-gray-500 uppercase">Managers</p>
            ...
            <p className="mt-2 text-2xl font-semibold">{managerCount}</p>
            <p className="mt-1 text-[11px] text-gray-500">Active manager accounts</p>
          </div>
          <div className={cardClass}>
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium tracking-wide text-gray-500 uppercase">Workers</p>
              <span className="inline-flex items-center justify-center w-8 h-8 text-xs font-semibold text-indigo-700 bg-indigo-100 rounded-full">
                WK
              </span>
            </div>
            <p className="mt-2 text-2xl font-semibold">{workerCount}</p>
            <p className="mt-1 text-[11px] text-gray-500">All registered workers</p>
          </div>
        </div>
      </div>

      {/* Worker type summary */}
      <div className="p-4 border rounded-2xl bg-gradient-to-r from-blue-50 via-white to-emerald-50 border-slate-100">
        <h2 className="mb-3 text-sm font-semibold text-gray-700">Workers by type</h2>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className={cardClass}>
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium tracking-wide text-gray-500 uppercase">First</p>
              <span className="inline-flex items-center justify-center w-8 h-8 text-xs font-semibold rounded-full bg-sky-100 text-sky-700">
                F1
              </span>
            </div>
            <p className="mt-2 text-2xl font-semibold">{firstWorkers}</p>
            <p className="mt-1 text-[11px] text-gray-500">FIRST level workers</p>
          </div>
          <div className={cardClass}>
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium tracking-wide text-gray-500 uppercase">Second</p>
              <span className="inline-flex items-center justify-center w-8 h-8 text-xs font-semibold rounded-full bg-amber-100 text-amber-700">
                F2
              </span>
            </div>
            <p className="mt-2 text-2xl font-semibold">{secondWorkers}</p>
            <p className="mt-1 text-[11px] text-gray-500">SECOND level workers</p>
          </div>
          <div className={cardClass}>
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium tracking-wide text-gray-500 uppercase">Third</p>
              <span className="inline-flex items-center justify-center w-8 h-8 text-xs font-semibold rounded-full bg-rose-100 text-rose-700">
                F3
              </span>
            </div>
            <p className="mt-2 text-2xl font-semibold">{thirdWorkers}</p>
            <p className="mt-1 text-[11px] text-gray-500">THIRD level workers</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border rounded-2xl bg-gradient-to-r from-amber-50 via-white to-rose-50 border-slate-100">
        ...
        {lowInventoryCount === 0 ? (
          ...
          <>
            ...
            <div className="mt-4 overflow-hidden text-xs bg-white border border-gray-200 rounded-xl">
