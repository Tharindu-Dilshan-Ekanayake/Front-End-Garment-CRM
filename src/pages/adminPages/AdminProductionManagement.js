...
export default function AdminProductionManagement() {
	...
	const handleDelete = async () => {
		...
		const ok = await deleteProduction(id);
		if (ok) {
			toast.success('Production deleted successfully');
			setDeleteTarget(null);
			// If the deleted one is open in the modal, close it
			if (selectedProduction && selectedProduction.taskId === id) {
				setIsModalOpen(false);
			}
		} else {
			...
			{deleteTarget && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
					<div className="w-11/12 max-w-sm p-5 bg-white rounded-lg shadow-2xl">
						<h3 className="mb-3 text-base font-semibold text-gray-900">Delete production</h3>
						<p className="mb-4 text-sm text-gray-700">
							Are you sure you want to delete production
							<span className="font-semibold"> #{deleteTarget.taskId}</span>?
						</p>
